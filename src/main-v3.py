# GovUpdate API v3.0 - With API Key Authentication
# No Playwright (due to Windows build issues) - Manual entry works for all sites
...
async def create_api_key(
    ...
    try:
        # Verify the requesting key is admin
        with get_db() as conn:
            key_data = conn.execute(
...
async def startup_event():
    ...
    print("Use this key to create additional API keys via /api/keys")
    print("=" * 70 + "\n")
...
if __name__ == "__main__":
    ...
    logger.info("Features: API key authentication, Manual entry")
